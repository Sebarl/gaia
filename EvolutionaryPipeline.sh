...
STARTTIME=$(date +%s)
./bin/ReleaseIntel/OwnPinballAE params.txt 1 ../Linux/Analyzer/stage2.weights ../Linux/Analyzer/framesToDrop.drop ../Linux/TestMovies/$(echo $1| cut -d"." -f1) $RAMCOUNT $1 > Objectives.weights
ENDTIME=$(date +%s)
...
STARTTIME=$(date +%s)
./bin/ReleaseIntel/OwnPinballAE params.txt 1 $RAMCOUNT ../../Analyzer/RAMMask.mask ../Linux/Analyzer/prefix ../../../ObjectInfererAE/Objectives.weights $1 > ../Linux/Solutions/$(echo $1| cut -d"." -f1)/player$DATE.solution
ENDTIME=$(date +%s)
