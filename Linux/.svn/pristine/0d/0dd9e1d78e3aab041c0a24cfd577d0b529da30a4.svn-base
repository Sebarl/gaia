...
static void UpdateGP(int w, void *data, int arg)
{
	...
	{			//FatRatKnight: Assume lua is always good. If it's doing nothing in particular using my logic, it'll pass-through the values anyway.
		...
		joy[0] = runAI(RAM, exitNow);
		//joy[2] = *(uint32 *)joyports[0].ptr >> 1
	}
	else
	{
		#ifdef _S9XLUA_H
...
static INPUTC GPCS={ReadGP,0,StrobeGP,UpdateGP,0,0,LogGP,LoadGP};
...
void InitializeInput(void)
{
	...
	//SetInputStuff(0);
	joyports[0].driver=&GPCS;
	joyports[0].type=SI_GAMEPAD;
	//joyports[1].driver=&GPCS;
	//joyports[1].type=SI_GAMEPAD;
	SetInputStuff(1);
	SetInputStuffFC();
}
