...
uint8 (*runAI)(uint8*,bool&);
void (*printStatistics)();
int (*initializeParams)(std::string);
...
*/
	...
	g_config->addOption('e',"ae-settings","Evolutive.Algorithm.Settings","ae.txt");
	g_config->addOption('l',"frame-limit","FrameLimit",-1);
	g_config->addOption('a',"analyze","AnalyzeMovie", "");
    g_config->addOption('d',"dump","DumpRam", "");
  ...
  if(romIndex >= 0)
	...
	*/
...
#else
	//attempt to load AI library
	void *handle;
	char *error2;
	std::string path;
	std::string ramPath;
	g_config->getOption("AnalyzeMovie", &path);
	g_config->getOption("DumpRam", &ramPath);
	if(!path.empty()){
         handle=dlopen("./Analyzer.so",RTLD_LAZY);
          if(!handle){
            printf("%s","Error, couldn't load Analyzer library");
            return 1;
            }
          *(void **)(&runAI) = dlsym(handle,"runAI");
            if((error2=dlerror())!=NULL){
                printf("%s", error2);
                return 2;
            }
          *(void **)(&printStatistics) = dlsym(handle,"printStatistics");
            if((error2=dlerror())!=NULL){
                printf("%s", error2);
                return 2;
            }
         *(void **)(&initializeParams) = dlsym(handle,"initializeParams");
                if((error2=dlerror())!=NULL){
                    printf("%s", error2);
                    return 2;
                }
	}
	else if(!ramPath.empty()){
             handle=dlopen("./Dumper.so",RTLD_LAZY);
	  if(!handle){
		printf("%s","Error, couldn't load Dump library");
		return 1;
		}
	  *(void **)(&runAI) = dlsym(handle,"runAI");
		if((error2=dlerror())!=NULL){
	 ...
	 *(void **)(&initializeParams) = dlsym(handle,"initializeParams");
     ...
     handle=dlopen("./AI.so",RTLD_LAZY);
	  if(!handle){
		printf("%s","Error, couldn't load AI library");
		return 1;
		}
	  *(void **)(&runAI) = dlsym(handle,"runAI");
		if((error2=dlerror())!=NULL){
	...
	//check if we have a frame limit
	int limit;
	g_config->getOption("FrameLimit",&limit);

	//try to load settings
	std::string aeParamsPath;
	if(path.empty()){
        g_config->getOption("Evolutive.Algorithm.Settings",&aeParamsPath);
        if(!initializeParams(aeParamsPath)){
            return 3;
        }

	}
	else{
        if(!initializeParams(path)){
            return 3;
        }
	}
	if(limit>-1){
