...
int main(int argc, char *argv[])
{
  ...
  // these six lines will still print the help output
	...
	g_config->addOption('I',"enable-ia","EnableIA",1);
	g_config->addOption('P',"play-dump","PlayDump","");
  ...
  if(romIndex >= 0)
	...
	//disable ai?
	void *handle;
	g_config->getOption("EnableIA",&IA);
	std::string path;
	g_config->getOption("PlayDump", &path);
	if(IA == 1){
	...
	//attempt to load Dump Player library

        char *error2;
             handle=dlopen("./DumpPlayer.so",RTLD_LAZY);
              if(!handle){
